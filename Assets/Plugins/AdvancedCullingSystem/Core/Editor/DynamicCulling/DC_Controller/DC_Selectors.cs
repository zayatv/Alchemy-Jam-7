    ...
    public abstract class DC_SelectorTemplate1 : DC_BaseSelector
    {
        ...
        protected void AssignAuto()
        {
            ...
            for (int i = 0; i < gos.Length; i++)
            {
                ...
                if (BaseChecksBeforeAssignIteration(go) && AssignIteration(go))
                    count++;
            }
        ...
        protected bool BaseChecksBeforeAssignIteration(GameObject current)
        {
            if (!current.activeInHierarchy)
                return false;

            if (current.TryGetComponent<DC_IgnoreByAssign>(out _))
                return false;
