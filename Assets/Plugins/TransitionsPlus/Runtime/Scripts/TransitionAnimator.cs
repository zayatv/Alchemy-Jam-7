...
namespace TransitionsPlus {
    ...
    public class TransitionAnimator : MonoBehaviour {
        ...
        static class ShaderParams {
            ...
            public static readonly int Pixelization = Shader.PropertyToID("_Pixelization");
            ...
            public const string SKW_PIXELIZATION = "PIXELIZATION";
        }
        ...
        void Start () {
            ...
            if (Application.isPlaying) {
                if (autoPlay) {
                    DelayedInvoke(StartTransition, playDelay);
                }
            }
        }
        ...
        void StartTransition () {
            ...
            if (profile != null && profile.sound != null) {
                DelayedInvoke(PlayAudio, profile.soundDelay);
            }
        }

        void DelayedInvoke (Action action, float waitTime) {
            StartCoroutine(DelayedInvokeCoroutine(action, waitTime));
        }

        IEnumerator DelayedInvokeCoroutine (Action action, float waitTime) {
            if (useUnscaledTime) {
        ...
        void LateUpdate () {
            ...
            if (autoFollow && followTarget != null && mainCamera != null) {
                ...
                if (profile.type.SupportsCentersCount() && profile.centersCount == 1) {
                    centers[0].x = center.x;
            ...
            if (t >= 1) {
                ...
                if (loadScene) {
                    // trigger events before scene is replaced
        ...
        public void UpdateMaterialProperties () {
            ...
            if (profile.pixelization > 0 && profile.type.SupportsPixelization()) {
                screenMat.SetFloat(ShaderParams.Pixelization, mainCamera.pixelWidth / (profile.pixelization + 1));
                screenMat.EnableKeyword(ShaderParams.SKW_PIXELIZATION);
            }
            else {
                screenMat.DisableKeyword(ShaderParams.SKW_PIXELIZATION);
            }

            UpdateMaterialRandomProperties();
        ...
        void UpdateMaterialRandomProperties () {
            int random;
            if (profile.randomize) {
                random = DateTime.Now.GetHashCode();
            }
            else {
                random = profile.seed;
            }
            screenMat.SetInt(ShaderParams.Seed, random);
        ...
        void CaptureCameras () {
            ...
            if (frameCount == lastCaptureFrame) return; // avoids multiple capture same frame
            lastCaptureFrame = frameCount;

            if (captureProgress == 0 || rtFirst == null) {
                CaptureFirstCameraTexture();
            }
            if (captureProgress == 0 || keepSecondCameraActive || rtSecond == null) {
        ...
        /// </summary>
        /// <param name="t">A value from 0 to 1</param>
        /// <param name="delayCameraCapture">If true, cameras will be captured in a few frames later. This option is only available in Unity Editor.</param>
        public void SetProgress (float t, bool delayCameraCapture = false) {
            t = Mathf.Clamp01(t);
            progress = t;
